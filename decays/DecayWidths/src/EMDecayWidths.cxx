...
double EMDecayWidths::execute(double ma_val, double mb_val, double mc_val, double ga_val, double sa_val,
  ...
  double thetak = 0; double phik = 0;

  double tensor1 = T1l(k_value, alpha_lam, alpha_rho,
		       MB, ML, thetak, phik, mLlA);
...
double EMDecayWidths::SPINFLIP_U3_GS_GS(double k_value, double alpha_lam,  double MB, double ML){
...
double EMDecayWidths::SPINFLIP_U1_1l_m1_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  ...
  double value = std::sqrt(6) * p_imag * MB * k_value * std::exp(value1 + value2 + value3) * std::sin(thetak)/ (4 * alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U1_1l_m0_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = std::sqrt(3) * p_imag * MB * k_value * std::exp(value1 + value2) * std::cos(thetak)/ (2 * alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U1_1l_m1m_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  ...
  double value = (-1.0) * std::sqrt(6) * p_imag * MB * k_value * std::exp(value1 + value2 + value3) * std::sin(thetak)/ (4 * alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U2_1l_m0_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = (-1.0) * std::sqrt(3) * p_imag * MB * k_value * std::exp(value1 + value2) * std::cos(thetak)/ (2 * alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U3_1l_m1_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  ...
  double value = (-1.0) * std::sqrt(6) * p_imag * ML * k_value * std::exp(value1 + value2) * std::sin(thetak)/ (2 * alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U3_1l_m0_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  ...
  double value = std::sqrt(3) * p_imag * ML * k_value * std::exp(value1) * std::cos(thetak)/ (alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U3_1l_m1m_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  ...
  double value2 = (-3.0) * std::pow(ML, 2) * std::pow(k_value, 2) / 2 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = std::sqrt(6) * p_imag * ML * k_value * std::exp(value1 + value2) * std::sin(thetak)/ (2 * alpha_lam  * (MB + 2 * ML) );
  return value;
}
...
double EMDecayWidths::SPINFLIP_U3_1r_m1_GS(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double phik, double thetak){
  k_value=0; alpha_lam=0; alpha_rho=0; MB=0; ML=0; phik=0;  thetak=0;
  double value = 0;
  return value * k_value * alpha_lam * alpha_rho * MB * ML * phik * thetak;
}
...
double EMDecayWidths::ORBITALSPLIT_U1_1l_m0_1l_m0(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
...
double EMDecayWidths::ORBITALSPLIT_U1_1l_m1m_1l_m1m(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = (value2 * std::pow(std::sin(thetak), 2) + 1) * k_value * std::exp(value1 + value2)/ (4 * alpha_rho);
  return value;
...
// U2_1lambda-1lambda
double EMDecayWidths::ORBITALSPLIT_U2_1l_m1_1l_m1(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = (value2 * std::pow(std::sin(thetak), 2) + 1) * k_value * std::exp(value1 + value2)/ (4 * alpha_rho);
  return value;
...
double EMDecayWidths::ORBITALSPLIT_U2_1l_m0_1l_m0(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = (((-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 4 * (std::pow(alpha_lam * (MB + 2. * ML), 2))) * std::pow(std::cos(thetak), 2) + 1) * k_value * std::exp(value1 + value2)/ (4 * alpha_rho);
  return value;
...
double EMDecayWidths::ORBITALSPLIT_U2_1l_m1m_1l_m1m(double k_value, double alpha_lam, double alpha_rho,  double MB, double ML, double thetak){
  double value1 = (-1.0) * std::pow(k_value, 2) / 8 * std::pow(alpha_rho, 2);
  double value2 = (-3.0) * std::pow(MB, 2) * std::pow(k_value, 2) / 8 * (std::pow(alpha_lam * (MB + 2. * ML), 2));
  double value = (value2 * std::pow(std::sin(thetak), 2) + 1) * k_value * std::exp(value1 + value2)/ (4 * alpha_rho);
  return value;
...
double EMDecayWidths::ORBITALSPLIT_U3_1l_m1_1l_m1(double k_value, double alpha_lam, double MB, double ML, double thetak){
  ...
  double value2 =  (2.) * std::pow(alpha_lam,2) * std::exp((3 * std::pow(k_value,2) * std::pow(ML,2))/(2. * std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))) * std::pow(MB + 2 * ML,2);
...
double EMDecayWidths::ORBITALSPLIT_U3_1l_m0_1l_m0(double k_value, double alpha_lam, double MB, double ML, double thetak){
  ...
  double value2 = std::pow(alpha_lam,2) * std::exp((3 * std::pow(k_value,2) * std::pow(ML,2)) / (2. * std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))) * std::pow(MB + 2 * ML,2);
  double value = value1/value2;
  return value;
}

double EMDecayWidths::ORBITALSPLIT_U3_1l_m1m_1l_m1m(double k_value, double alpha_lam, double MB, double ML, double thetak){
...
//U1 1lam->1rho
double EMDecayWidths::ORBITALSPLIT_U1_1l_m0_1r_m0(double k_value, double alpha_rho, double alpha_lam, double MB, double ML, double thetak){
  double value1 = std::pow(k_value,2) * MB * std::pow(std::cos(thetak),2);
  double value2 = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
...
//U2 1lam->1rho
double EMDecayWidths::ORBITALSPLIT_U2_1l_m0_1r_m0(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  double value1 = std::pow(k_value,2) * MB * std::pow(std::cos(thetak),2);
  double value2 = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
  double value2p = 4 * alpha_lam * alpha_rho * MB + 8 * alpha_lam * alpha_rho * ML;
  value2 = value2 * value2p;
...
//U1 1rho->1rho
double EMDecayWidths::ORBITALSPLIT_U1_1r_m1_1r_m1(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  ...
  double value2p = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
  value2 = value2 * value2p;
  double value = value1/value2;
  return value;
}

double EMDecayWidths::ORBITALSPLIT_U1_1r_m0_1r_m0(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
...
double EMDecayWidths::ORBITALSPLIT_U1_1r_m1m_1r_m1m(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  double value1 = 8 * std::pow(alpha_rho,2) - std::pow(k_value,2) + std::pow(k_value,2) * std::pow(std::cos(thetak),2);
  double value2 = 8. * std::pow(alpha_rho,2);
  double value2p = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
  value2 = value2 * value2p;
...
//U2 1rho->1rho
double EMDecayWidths::ORBITALSPLIT_U2_1r_m1_1r_m1(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  double value1 =(8 * std::pow(alpha_rho,2) - std::pow(k_value,2) + std::pow(k_value,2) * std::pow(std::cos(thetak),2));
  double value2 = 8. * std::pow(alpha_rho,2);
  double value2p = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
  value2 = value2 * value2p;
...
double EMDecayWidths::ORBITALSPLIT_U2_1r_m0_1r_m0(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  double value1 = (4 * std::pow(alpha_rho,2) - std::pow(k_value,2) * std::pow(std::cos(thetak),2));
  double value2 = 4. * std::pow(alpha_rho,2);
  double value2p = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
  value2 = value2 * value2p;
...
double EMDecayWidths::ORBITALSPLIT_U2_1r_m1m_1r_m1m(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  double value1 = 8 * std::pow(alpha_rho,2) - std::pow(k_value,2) + std::pow(k_value,2) * std::pow(std::cos(thetak),2);
  double value2 = 8 * std::pow(alpha_rho,2);
  double value2p = std::exp((std::pow(k_value,2) * (std::pow(alpha_rho,-2) + (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8.);
  value2 = value2 * value2p;
...
//U3 1rho->1rho
double EMDecayWidths::ORBITALSPLIT_U3_1r_m1_1r_m1(double k_value, double alpha_lam, double MB, double ML){
  double value = std::exp((-3 * std::pow(k_value,2) * std::pow(ML,2))/(2. * std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2)));
  return value;
}

double EMDecayWidths::ORBITALSPLIT_U3_1r_m0_1r_m0(double k_value, double alpha_lam, double MB, double ML){
  double value = std::exp((-3 * std::pow(k_value,2) * std::pow(ML,2))/(2. * std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2)));
  return value;
}

double EMDecayWidths::ORBITALSPLIT_U3_1r_m1m_1r_m1m(double k_value, double alpha_lam, double MB, double ML){
  double value = std::exp((-3 * std::pow(k_value,2) * std::pow(ML,2))/(2. * std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2)));
  return value;
}
...
double EMDecayWidths::ORBITALSPLIT_U1_1r_m0_1l_m0(double k_value, double alpha_lam, double alpha_rho,
  ...
  double value1 = std::exp(-std::pow(k_value,2)/(8. * std::pow(alpha_rho,2)) -
			   (3 * std::pow(k_value,2) * std::pow(MB,2))/(8. * std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2)) + p_imag * phik);
...
double EMDecayWidths::ORBITALSPLIT_U2_1r_m0_1l_m0(double k_value, double alpha_lam, double alpha_rho,
  ...
  double value1 = std::exp(-(std::pow(k_value,2) * (std::pow(alpha_rho,-2) +
						    (3 * std::pow(MB,2))/(std::pow(alpha_lam,2) * std::pow(MB + 2 * ML,2))))/8. + p_imag * phik);
  double value1p = std::pow(k_value,2) * MB * std::cos(thetak) * std::sin(thetak);
  value1 = value1 * value1p;
  double value2 = 4 * alpha_lam * alpha_rho * MB + 8 * alpha_lam * alpha_rho * ML;
  double value = value1/value2;
...
double EMDecayWidths::ORBITALSPLIT_U1_2l_m0_GS(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  ...
  double value2 = 16.*std::pow(alpha_lam,2)*std::exp((std::pow(k_value,2)*(std::pow(alpha_rho,-2) + (3*std::pow(MB,2))/(std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2))))/8.);
...
double EMDecayWidths::ORBITALSPLIT_U3_2l_m0_GS(double k_value, double alpha_lam, double MB, double ML, double thetak){
  ...
  double value2 = 4.*std::pow(alpha_lam,2)*std::exp((3*std::pow(k_value,2)*std::pow(ML,2))/(2.*std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2)));
  double value2p = std::pow(MB + 2*ML,2);
  value2 = value2 * value2p;
  double value = value1/value2;
  return (-1.0) * std::sqrt(3) * value;  
...
double EMDecayWidths::ORBITALSPLIT_U1_2r_m0_GS(double k_value, double alpha_lam, double alpha_rho, double MB, double ML, double thetak){
  ...
  double value2 = 8.*std::sqrt(3)*std::pow(alpha_rho,2)*std::exp((std::pow(k_value,2)*(std::pow(alpha_rho,-2) + (3*std::pow(MB,2))/(std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2))))/8.);
  double value = value1/value2;
  return value;
}
...
// U1 1nlam->gs
double EMDecayWidths::ORBITALSPLIT_U1_1nl_m0_GS(double k_value, double alpha_lam, double alpha_rho, double MB, double ML){
  double value1 = std::pow(k_value,2)*std::pow(MB,2);///
  double value2 = 4.*std::pow(alpha_lam,2) * std::exp((std::pow(k_value,2)*(std::pow(alpha_rho,-2) + (3*std::pow(MB,2))/(std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2))))/8.);
  double value2p = std::pow(MB + 2*ML,2);
  value2 = value2 * value2p;
  double value = value1/value2;
  return std::sqrt(1.5)*value;
...
// U2 1nlam->gs
double EMDecayWidths::ORBITALSPLIT_U2_1nl_m0_GS(double k_value, double alpha_lam, double alpha_rho, double MB, double ML){
  double value1 = std::pow(k_value,2)*std::pow(MB,2);///  
  double value2 = 4.*std::pow(alpha_lam,2) * std::exp((std::pow(k_value,2)*(std::pow(alpha_rho,-2) + (3*std::pow(MB,2))/(std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2))))/8.);
  double value2p = std::pow(MB + 2*ML,2);
  value2 = value2 * value2p;
...
// U3 1nlam->gs
double EMDecayWidths::ORBITALSPLIT_U3_1nl_m0_GS(double k_value, double alpha_lam, double MB, double ML){
  double value1 = std::pow(k_value,2)*std::pow(ML,2);///
  double value2 = std::pow(alpha_lam,2)*std::exp((3*std::pow(k_value,2)*std::pow(ML,2))/(2.*std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2)));
  double value2p = std::pow(MB + 2*ML,2);
  value2 = value2 * value2p;
  double value = value1/value2;
  return std::sqrt(1.5)*value;
...
// U1 1nrho->gs
double EMDecayWidths::ORBITALSPLIT_U1_1nr_m0_GS(double k_value, double alpha_lam, double alpha_rho, double MB, double ML){
  double value1 = std::pow(k_value,2);///
  double value2 = 4.*std::sqrt(6)*std::pow(alpha_rho,2)*std::exp((std::pow(k_value,2)*(std::pow(alpha_rho,-2) + (3*std::pow(MB,2))/(std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2))))/8.);
  double value = value1/value2;
  return value;
}

// U2 1nrho->gs
double EMDecayWidths::ORBITALSPLIT_U2_1nr_m0_GS(double k_value, double alpha_lam, double alpha_rho, double MB, double ML){
  double value1 = std::pow(k_value,2);///
  double value2 = 4.*std::sqrt(6)*std::pow(alpha_rho,2)*std::exp((std::pow(k_value,2)*(std::pow(alpha_rho,-2) + (3*std::pow(MB,2))/(std::pow(alpha_lam,2)*std::pow(MB + 2*ML,2))))/8.);
  double value = value1/value2;
  return value;
...
double EMDecayWidths::T1l(double k_value, double alpha_lam, double alpha_rho,
  ...
  double value2 = ORBITALSPLIT_U1_2l_m0_GS(k_value, alpha_lam, alpha_rho, MB, ML, thetak);
  double value3 = SPINFLIP_U1_GS_GS(k_value, alpha_lam, alpha_rho,  MB, ML);
  double value4 = ORBITALSPLIT_U1_1nl_m0_GS(k_value, alpha_lam, alpha_rho, MB, ML);
...
double EMDecayWidths::T2l(double k_value, double alpha_lam, double alpha_rho,
			  double MB, double ML, double thetak, double phik, double mLlA){
  double value1 = ORBITALSPLIT_U2_1l_m1_1l_m1(k_value, alpha_lam, alpha_rho, MB, ML, thetak);
  double value2 = ORBITALSPLIT_U2_2l_m0_GS(k_value, alpha_lam, alpha_rho, MB, ML, thetak);
  double value3 = SPINFLIP_U2_GS_GS(k_value, alpha_lam, alpha_rho,  MB, ML);
  double value4 = ORBITALSPLIT_U2_1nl_m0_GS(k_value, alpha_lam, alpha_rho, MB, ML);
  double value = value1 + std::sqrt(1/3) * value2 +  value3 +  std::sqrt(2/3) * value4;
  return p_imag * std::sqrt(1/6) * alpha_lam * value;
}
...
double EMDecayWidths::T3l(double k_value, double alpha_lam, double alpha_rho,
			  double MB, double ML, double thetak, double phik, double mLlA){
  double value1 = ORBITALSPLIT_U3_1l_m1_1l_m1(k_value, alpha_lam, MB, ML, thetak);
  double value2 = ORBITALSPLIT_U3_2l_m0_GS(k_value, alpha_lam, MB, ML, thetak);
  double value3 = SPINFLIP_U3_GS_GS(k_value, alpha_lam, MB, ML);
  double value4 = ORBITALSPLIT_U3_1nl_m0_GS(k_value, alpha_lam, MB, ML);
  double value = value1 + std::sqrt(1/3) * value2 +  value3 +  std::sqrt(2/3) * value4;
...
double EMDecayWidths::T1r(double k_value, double alpha_lam, double alpha_rho,
  ...
  double value1 = ORBITALSPLIT_U1_1r_m1_1r_m1(k_value, alpha_lam, alpha_rho, MB, ML, thetak);
...
double EMDecayWidths::T2r(double k_value, double alpha_lam, double alpha_rho,
			  double MB, double ML, double thetak, double phik, double mLrA){
  double value1 = ORBITALSPLIT_U2_1r_m1_1r_m1(k_value, alpha_lam, alpha_rho, MB, ML, thetak);
  double value2 = ORBITALSPLIT_U2_2r_m0_GS(k_value, alpha_lam, alpha_rho, MB, ML, thetak);
  double value3 = SPINFLIP_U2_GS_GS(k_value, alpha_lam, alpha_rho,  MB, ML);
  double value4 = ORBITALSPLIT_U2_1nr_m0_GS(k_value, alpha_lam, alpha_rho, MB, ML);
  double value = value1 + std::sqrt(1/3) * value2 +  value3 +  std::sqrt(2/3) * value4;
