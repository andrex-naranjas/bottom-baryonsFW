...
class FlavorAmplitudes():
    ...
    def __init__(self, baryons):
        ...
        self.u = Matrix([[1],[0],[0],[0]]) # up quark state
        self.d = Matrix([[0],[1],[0],[0]]) # down quark state
        self.s = Matrix([[0],[0],[1],[0]]) # strange quark state
        self.b = Matrix([[0],[0],[0],[1]]) # bottom quark state
    ...
    def magnetic_operators(self, index): #mu(i)
        """
    ...
    def tensor_product_operator_1(self, index=1, dim=4): # Mu(1):
        """
    ...
    def tensor_product_operator_2(self, index=2, dim=4): # Mu(2):
        """
    ...
    def tensor_product_operator_3(self, index=3, dim=4): # Mu(3):
        """
    ...
    def flavor_states(self, state_a, state_b, state_c): # Fs
        """
    ...
    def flavor_state(self, state_a, state_b, state_c): # Fs
        """
        Method to calculate the flavor states
        state_a,b,c should be sympy matrices of the form: Matrix([[0],...,[1]])
        """
    ...
    def transition_lambda0_sigma0(self):
        ...
        """
        return conjugate(transpose(self.Lambda_0))*TensorProduct(TensorProduct(self.tensor_product_operator_1(index=1, dim=4), self.identity_matrix(dim=2)), self.identity_matrix(dim=2))*self.Sigma_0