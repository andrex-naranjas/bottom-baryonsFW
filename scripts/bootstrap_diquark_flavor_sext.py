...
def model(is_omega, is_cascade_p, is_sigma, v, w, x, y, z, md1, md2, md3, mb, k, a, b, e, g):
    return is_omega*md1 + is_cascade_p*md2 + is_sigma*md3 + mb +\
        v*k*np.sqrt(1./( ( is_omega*md1 + is_cascade_p*md2 + is_sigma*md3)*mb / ( is_omega*md1 + is_cascade_p*md2 + is_sigma*md3 + mb ) ) ) +\
        w*a + x*b + y*e + z*g
...
def least_squares(md1, md2, md3, mb, k, a, b, e, g):
    ...
    yvar_2 = 0.001
    pred_m = model(param_is_omega, param_is_cascade_p, param_is_sigma,
                   param_v, param_w, param_x, param_y, param_z,
...
def fit(least_squares):
    m = Minuit(least_squares, md1=900, md2=300, md3=300, mb=1400, k=0, a=0, b=0, e=0, g=0)
    m.limits['mb'] = (4900, 6000)
    # m.limits['md1'] = (850,950)
    # m.limits['md2'] = (650,850)
    # m.limits['md3'] = (500,700)
    m.limits['md1'] =  (850, 950) #(500, 1000)   #  #omega (500, 1500)
    m.limits['md2'] =  (650, 850) #(500, 1000)   #  #cascade prime
    m.limits['md3'] =  (500, 700) #(500, 1000)   #  #sigma
